...
$uc_sql = "SELECT uc.ucid, c.coupon_name, c.coupon_price FROM user_coupons uc
  ...
  WHERE c.status = 2 AND uc.status = 1 AND uc.userid = '$userid' AND uc.use_max_date >= now()";
//coupons 테이블의 cid와 user_coupons 테이블의 couponid가 일치하는 것을 조건으로 coupons 테이블과 user_coupons 테이블을 합치는데(join) 다음 조건에 부합하는 데이터만 추려서 합친다.
//합친 데이터에서 쿠폰이 사용이 사용가능하고(c.status = 2), 사용한 적이 없고(uc.status = 1), 사용기한이 남아있는(uc.use_max_date >=now()) 행을 조회
                            ...
                            <table class="table table-responsive">
                                ...
                                if(isset($cartArr)){
                                  foreach($cartArr as $cart){
                                    $total += ($cart->price) * $cart->cnt;
                                ?>
                                    <tr>
                                        <td class="cart_product_img d-flex align-items-center">
                                        <input type="hidden" name="pid[]" id="pid" value="<?= $cart->pid ?>">
                                        <input type="hidden" name="cart_name[]"  value="<?= $cart->name ?>">
                                        <input type="hidden" name="cart_cnt[]" class="cart_cnt" value="<?= $cart->cnt ?>">
                                        <input type="hidden" name="cart_total[]" class="cart_subtotal" value="">
                                            <a href="#"><img src="<?= $cart->thumbnail ?>" alt="Product"></a>
                                            <h6><?= $cart->name ?></h6>
                                        </td>
                                        <td class="price"><span class="number" data-price="<?= $cart->price ?>"><?= $cart->price ?></span>원</td>
                                        <td class="qty">
                                            <div class="quantity">
                                        ...
                                        <td class="total_price"><span class="number">0</span>원
        ...
        <!-- ****** Cart Area End ****** -->
         <script>
          ...
          $('#coupon-select').change(function(){
            let ucid = $(this).val();
            let ucprice = Number($(this).find('option:selected').attr('data-price'));
