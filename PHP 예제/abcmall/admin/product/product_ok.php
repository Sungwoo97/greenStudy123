...
/* if($_POST['cate1']){   //if 조건문
  $cate1 = $_POST['cate1'];
}else{
  $cate1 = '';
}*/
// $cate1 = $_POST['cate1'] ? $_POST['cate1'] : ''; //삼항 연산자
/*
...
$cate1 =  $_POST['cate1'] ?? '';  //null 병합 연산자
$cate2 =  $_POST['cate2'] ?? '';  //null 병합 연산자
$cate3 =  $_POST['cate3'] ?? '';  //null 병합 연산자
...
$product_image_id = $_POST['product_image']; // 추가 이미지의 id들
$product_image_id = rtrim($product_image_id, ','); // 추가 이미지의 id들
...
$result= $mysqli->query($sql);  //product 테이블에 상품정보 입력(정보)
$pid = $mysqli->insert_id; //상품 등록시 자동으로 생성되는 상품의 고유번호를 미리 확인
if($result){  //상품이 product 테이블에 등록되면
  if($product_image_id){  // 11,12 테이블 product_image_table에서 imgid의 값이 11,12인 데이터 행ㅇ에서 pid 값을 $pid로 업데이트
