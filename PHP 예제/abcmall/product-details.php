...
//저장한 분리하여 저장한 category 값을 각각 조회
$cate_sql = "SELECT name, code FROM category WHERE code IN ('$cate1', '$cate2', '$cate3')";
$cate_result = $mysqli->query($cate_sql);

//조회한 값을 $category_names에 각각 저장
$category_infos = [];
while ($cate_data = $cate_result->fetch_object()) {
    $category_infos[] = $cate_data;
}
/*
$rvArr = [10, 12];
print_r(gettype($rvArr));   // 배열
$rvStr = json_encode($rvArr);
echo gettype($rvStr);       // 문자열
*/
$rvArr = [];

if(isset($_COOKIE['recent_viewed'])){
  $rvArr = json_decode($_COOKIE['recent_viewed']);//기존 쿠키의 값을 조회해서 변수 할당 // 문자열 -> 배열
  if(!in_array($pid, $rvArr)){  //이미 본 상품이 아니라면, 새상품을 열었다면
    if(sizeof($rvArr)>=3){  //이미 3개 이상이면
      array_shift($rvArr);
    }
    array_push($rvArr, $pid);
    $rvStr = json_encode($rvArr); // 배열을 문자열로 바꿔줌
    setcookie('recent_viewed', $rvStr, time()+86400, "/");
  }
  /*
    배열에서 특정 값이 있는지 확인해서 T/F반환 => in_array(찾을 값, 대상)
    이미 본 상품이 아니라면 
      본 상품이 3개 이상이라면 첫번째 상품을 제거
      쿠키에 상품정보를 추가 (array_push)
  */
  //setcookie('recent_viewed', '상품번호', time()+86400, "/");
}else{
  array_push($rvArr, $pid);
  $rvStr = json_encode($rvArr); // 배열을 문자열로 바꿔줌
  setcookie('recent_viewed', $rvStr, time()+86400, "/");
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="single_product_desc">
                            ...
                            <form class="cart clearfix mb-50" id="cartform" method="post">
                            <?php 
                            ...
                            <div class="widget size mb-50">
                                ...
                                <ul class="d-flex gap-3 list-unstyled" >
                                  <li class="w-5">
                                    ...
                                    <?php echo $ok==0? 'checked' : ''; ?> 
                                    >
                                  </li>
                                  <li class="w-5">
                                    <label for="option1_<?=$op->poid ?>"><?= $op->option_name;?></label>
                                  </li>
                            ...
                            <div class="widget size mb-50">
                                ...
                                <div class="widget-desc">
                                <?php               
                                foreach($options2 as $ok=> $op){              
                                ?> 
                                <ul class="d-flex gap-3 list-unstyled" >
                                <li class="w-5">
                                    <input type="radio" name="option2" id="option2_<?=$op->poid ?>" value="<?= $op->option_name;?>" data-price="<?= $op->option_price ?>" <?php echo $ok==0? 'checked' : ''; ?> >
                                  </li>
                                  <li class="w-5">
                                    <label for="option2_<?=$op->poid ?>"><?= $op->option_name;?></label>
                                  </li>
